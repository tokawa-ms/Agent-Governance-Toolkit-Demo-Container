...
- **Overview page (`/`)**: Summarizes the demo and its four-stage governance flow, then links to the dedicated demo page.
- **Demo tab, left pane (35%)**: On `/demo`, select a predefined scenario, run it, and inspect the current decision and output summary.
- **Demo tab, right pane (65%)**: Follow Request, Governance Gate, Tool Execution, and Result as the run progresses. Denied requests clearly show that execution was blocked. The lower section always shows the applicable gate and reviewed C# excerpt, then highlights the evaluation, deny branch, and early return traversed by a completed run.
...
- **Multilingual UI**: Japanese (`ja-JP`) is the default language. The language dropdown remains visible in the upper-right corner and supports `en-US`, `ja-JP`, `zh-TW`, `zh-CN`, `zh-HK`, and `ko-KR`. The selected language is stored in a cookie and remains active across navigation and browser reloads.
- Governance events enter a single-reader queue with a capacity of 1,024 and are appended to a daily Append Blob with up to three attempts. To avoid blocking the synchronous callback, a new event is rejected when the queue is full and audit storage is marked failed.
...
At runtime, the application loads `src/AgentGovernanceDemo/policies/default.yaml`; the file is copied to build and publish output. The code also retains an equivalent default policy for fallback and the explanatory UI. Deny takes precedence when rules conflict. Even the three allowed tools never call the network, operating system, file system, or Azure APIs; they return fixed strings only.

The C# excerpt in the right pane is reviewed explanatory data that remains available after publish. It follows the live Toolkit decision and execution state, and tests detect drift in its important lines.
...
Open `https://localhost:7011` for the overview or `https://localhost:7011/demo` for the demo. Japanese is displayed by default; use the upper-right dropdown to switch among the six supported languages. The selection is retained for later visits. The HTTPS launch profile also binds `http://localhost:5101` for local development. If the development HTTPS certificate is not trusted, run `dotnet dev-certs https --trust` as needed.
...
When values are omitted, the Storage URI and container name in `appsettings.json` are used. The default `https://devstoreaccount1.blob.core.windows.net/` is a disconnected-demo placeholder; it does not start or configure Azurite. The UI and policy decisions still work, but Blob reads and writes report a failure state. Telemetry is disabled when `APPLICATIONINSIGHTS_CONNECTION_STRING` is absent, and the UI reports a configuration error when it is malformed. Never put connection strings, credentials, or tokens in the README, source code, or logs.
...
| `Storage__RecentRecordLimit`            | Optional                  | Audit records restored and retained by the UI; default: 100, maximum: 500 |
| `Demo__MaxRunsPerMinute`                | Optional                  | Per-browser-session run limit; default: 8                                 |
| `Demo__StepDelayMilliseconds`           | Optional                  | Display delay between the four stages; default: 450 ms, maximum: 5,000 ms |
...
Open `http://localhost:8080` when running with Docker. `--provenance=false` prevents an OCI provenance index that Azure Container Apps may not recognize as a single Linux amd64 image and matches the CI container build. To pass Azure configuration, use a local secret-management mechanism such as `--env-file` rather than putting secrets directly in shell history.
...
The Bicep deployment disables the ACR admin user, and the Standard app pulls images only through its user-assigned managed identity.
